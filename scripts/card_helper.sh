...
# vcf 檔名一律只取英文（v0.19.x）：英文名欄位可能中英混填（例「Ming Wang 小明」），
# .vcf 檔名 + 上傳 URL + QR 只用 ASCII 英文部分（→ MingWang.vcf），中文絕不進檔名。
...
def to_card_mobile(s):
    ...
    # 空格/dash）一律輸出 +886-9XX-XXX-XXX（3-3-3），例 0909050269 → +886-909-050-269。
    # 修正舊版只拆末 6 碼、簽呈手機無空格時中間組沒切開（→ +886-909050-269）的問題。
    import re as _re
