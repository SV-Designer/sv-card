...
本檔案記錄 claude-sv-card 的變更歷史。格式參考 [Keep a Changelog](https://keepachangelog.com/zh-TW/1.1.0/)。
...
### Changed
- **更名 `sv-card` → `claude-sv-card`**：兩帳號 GitHub repo（個人 chensoo8911 + 公司 SV-Designer）、本機兩邊 clone 資料夾、active skill 資料夾與內部 4 條 symlink、`SKILL.md` name 欄位、`~/.claude/settings.json` 8 條權限白名單、`sv-card-release.sh` 的 ME/SV 路徑變數、git hook `_sync-templates.sh` 的 REPO 路徑、全域 CLAUDE.md 路由表，全部同步改名。CHANGELOG 內的歷史路徑提及保留原樣（準確描述當時狀態）。

### Changed
- **統一 CHANGELOG 裡混用的測試假名**：幾處「實測某簽呈踩到 bug」的歷史敘述使用了不同的測試假名（White Chen、劉琪琪、黃阿福/Fu Huang、林小芳、Strong Wu/Wo）——**這些本來就是假名，非真實姓名殘留**，為求全 repo 一致性統一改用既有慣例「王小明 / Ming Wang」（含 typo 示範情境：Strong Wo→Wu 改為語意相同的 Ming Wan→Ming Wang）。docs/pdf-extract.md、docs/SOP.md 裡同款用詞一併同步。

### Fixed
- **同批次一併換掉同一起（`王小美`／`王小美`）事故裡的真實電話 `0909050269`**，改為 repo 統一假號 `0909050269`。
- **`tests/fixtures/sidecar_valid_zhongzi_taiwan.json` 內部資料不一致**：`fields` 用了假名「徐樂天」，但 `dest_path` 卻是另一個假名「劉琪琪」（新舊測資交替時留下的不一致，非真實姓名問題）。已同步改為 `徐樂天_Chi Liu`，與 `fields` 一致。
- **`templates/20260715-名片模版_{中子BVI,台灣中子}（無手機版）.ai` 殘留最早那組真實電話 `0909050269`**：這兩份模板是 2026-07-15 新增，晚於 2026-07-11 的原始真號清理批次，未被涵蓋到（先前 v0.24.0 只補了 `%%For` 帳號名欄位，漏了電話號碼本體）。等長二進位取代為 `0909050269`（皆 10 碼，檔案位元組長度不變、結構未受影響）。
- 起因：v0.24.3/v0.24.4 的護欄複掃只涵蓋 `.md`/`.py`/`.sh`，用 `git grep`（涵蓋所有 tracked 檔案類型，不限副檔名）才連 `tests/*.json` 與 `.ai` binary 都掃到殘留（電話號碼部分屬實、假名部分事後確認並非真實姓名）。教訓見 [[feedback_sv_card_dual_push]]：**push 公開 repo 前的護欄要用 `git grep` 掃全部 tracked 檔案，不能只挑常見副檔名；且「像真名的樣式」不代表真的是真名，需與使用者核實再下定論**。

## [0.24.3] — 2026-07-16

### Fixed
- **修正公開版 SOP.md 洩漏真實同仁姓名**：「經典復刻款 BVI 測試紀錄」段落的真實姓名改為中性描述「某同仁申請」。此洩漏源於 v0.24.2 gh-sv 歷史重置時，鏡射流程只掃了已知樣式（帳號名／電話），沒涵蓋「同仁真名當測試紀錄」這類例子，這次補上並重置公開版歷史清除該筆紀錄。
- **SKILL.md／docs/pdf-extract.md／docs/SOP.md 與 CHANGELOG 歷史脫節的落差**：
  - `SKILL.md` 完全遺漏「經典復刻款BVI」第四支援版型（僅列三種），會導致真的遇到此版型時被誤判「未支援、停下問」——現已補上路由，明確標示此版型走 `docs/SOP.md` 專屬手動流程，不進自動化 Step。
  - `docs/pdf-extract.md` 欄位對照表與停下判斷清單同步遺漏第四版型，現已補齊、與既有旗標說明對齊。
  - `docs/SOP.md`「🎯 觸發指令」段落仍保留 v0.20.0 已廢止的四句觸發語＋版型交叉檢核機制，現已刪除、對齊現行「僅認『做名片』、版型只看簽呈欄位」行為。

### Removed
- **清除 `card_helper.sh` 內棄用的 `classic-bvi` 半自動化程式碼**：`--template-type classic-bvi`、`--title-en`、`finalize-classic` 子命令、孤兒函式 `format_mobile_local`。此分支曾於 v0.19.0 測試，後續決策「暫不做 card_helper 自動化」（配色需人眼確認、全自動 ROI 低），程式碼留存但未清理；本次確認不影響其餘版型（tw/zhongzi-bvi/zhongzi-taiwan）功能後移除。經典復刻款 BVI 維持手動流程，不受影響（見 `docs/SOP.md` 專屬章節）。
- `extract_signoff_fields.py` 的四版型白名單維持不變（經典復刻款BVI 仍是合法辨識版型，只是不再對應任何 card_helper.sh 自動化參數）。
...
- **`extract-pdf` 腳本階段主動偵測三種非常規欄位並印 ⚠️**（`extract_signoff_fields.py`）：把原本只寫在 docs「Claude 必看項」、靠 Claude 臨場眼力抓的判斷**機械化**，讓每張簽呈在萃取當下就自動示警，不再漏看。JSON output 新增三個旗標，main() 依旗標對 stderr 印 ⚠️：
  - `english_name_has_cjk`：英文名欄位含中文（中英混填，如「Ming Wang 小明」）→ 🛑 停下問使用者名片 PH_NAME_EN 要印「只英文／只中文／中英都印」（`.vcf`／URL／QR 已自動只取 ASCII 英文）。
...
### Changed
- **`.vcf` 檔名／上傳 URL／QR 一律只取 ASCII 英文**（`card_helper.sh` `to` 前 vcf_name 衍生）：英文名欄位中英混填時（例「Ming Wang 小明」），舊版 `en.replace(" ","")` 會殘留中文（`MingWang小明.vcf`）。此版先剝除非 ASCII 字元再去空格 → `MingWang.vcf`，讓檔名／`drive.streetvoice.com/vcard/` URL／QR 掃出的網址一致乾淨。**名片顯示 PH_NAME_EN 不受影響**（仍用完整 `--english` 值，中英混填照印）。純中文英文名（無 ASCII）→ fallback 保留原值，交由 SOP 停下問。
- **手機一律 3-3-3 格式 `+886-9XX-XXX-XXX`**（`to_card_mobile`）：舊版只把末 6 碼拆 3+3，簽呈手機無空格時（例 `0909050269`）中間組沒被切開 → 產生 `+886-909050-269`。此版改為：標準台灣手機（10 碼 09 開頭，不論有無空格/dash/國碼）一律 normalize 成 `+886-909-050-269`；非標準號碼沿用舊邏輯。
- **中文姓名去「(數字)」擴為全/半形**（`extract_signoff_fields.py`）：舊版 `\(\d+\)` 只吃半形括號＋半形數字。此版改 `[（(][\d０-９]+[）)]` 並去尾端空白，全形括號／全形數字（例「王小明（４５４）」）也能正確去除，確保員編絕不進 PH_NAME。

### Notes
- 三項皆為 2026-07-14 製作測試名片時發現的實例修正：英文名「Ming Wang 小明」中英混填、姓名「王小明(454)」帶員編、手機 `0909050269` 無空格。
...
- **經典復刻款 BVI 版 template 納入 repo（測試中，未畢業）**：`templates/20260612-王小明_經典款.ai`（雙面卡、米褐色系）。`docs/SOP.md` 新增「經典復刻款 BVI 版分支」節，完整記錄此款專屬處理：
  ...
  - 各版型畢業狀態表新增此款（測試中，表單 646/王小明 第 1 次跑通，需第 2 次才畢業）。
...
- **台灣中子版 + 中子 BVI 版雙雙納入自動化白名單**（各跑通 2 次後畢業）：
  - 台灣中子：v0.12.0 表單號 647／王小明第 1 次 + 本版同簽呈第 2 次。
...
### Notes
- 台灣中子版屬**新款測試階段**（依 memory `feedback_new_card_type_testing`）：初期每步先停下確認，成功跑 ≥ 2 次後才討論加入自動化白名單。本版已用簽呈（表單號 647 / 王小明）測試 1 次通過（原檔 / JPG / OL-CS6 / 簽呈備份皆正確）
...
### 設計動機
- 跑王小明中子 BVI 簽呈（表單號 647）時連續踩到 4 個 bug：
...
- **回歸測試**：對 #554（王小明 / Ming Wang）簽呈 PDF 跑 `extract-pdf` 比對前後輸出，diff 為空 — 改動無 regression
...
- **`extract_signoff_fields.py` Bug A：全形 CJK 相容字符未回正**
  ...
  - 實測 #498（王小明）`title="平⾯設計"`、#554（王小明）`applicant_dept="執⾏董事辦公室"` / `surname_cn="⽯"` 都抓出怪字
...
### 設計動機
- **背景**：當天製作 #661 名片時實測抓到 PDF 上英文名是 typo「Ming Wan」（應為 `Wang`），Claude 視覺判斷後停下確認 — 證明人類視覺校驗有不可取代的價值
- **為何雙重檢核而非純腳本**：
...
### 設計動機
- 本次 Ming Wang 名片實測時使用者發現名片畫面手機開頭沒 +886，回看 sidecar 確認 `PH_PHONE_MOBILE` 寫的就是 `0909-050-269`（未轉），追到 line 213-214 Python 邏輯缺漏
