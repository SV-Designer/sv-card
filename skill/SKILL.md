...
在 Step 1 前，**先檢查 `~/.config/sv-card/env` 是否含 `SV_OUTPUT_CONFIRMED=1`**：

```bash
. ~/.config/sv-card/env 2>/dev/null
[ "$SV_OUTPUT_CONFIRMED" = "1" ] && echo "skip-step0" || echo "run-step0"
```

- 若印出 `skip-step0` → 直接跳到 Step 1
- 若印出 `run-step0` → 執行首次確認流程：
...
**b. 收到回答後，依序執行**：
```bash
# 假設使用者選 A 或回 "OK"，OUT="$HOME/Documents/SV-名片"
# 若選 B，OUT="<使用者輸入>"
mkdir -p "$OUT"
open "$OUT"   # 開 Finder 引導使用者看實際位置

# 更新 env 檔（atomic）
TMP="$HOME/.config/sv-card/env.tmp"
cat > "$TMP" <<EOF
# sv-card 使用者偏好（由首次製作名片流程寫入）
SV_OUTPUT_BASE="$OUT"
