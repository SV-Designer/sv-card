---
name: sv-card
description: StreetVoice 街聲名片自動化製作。觸發條件 = 使用者說「我要製作名片」或「做 SV 名片」或「幫我做 SV 名片」並提供簽呈 PDF。產出 5 個交付檔案（原檔 .ai / OL CS6 / JPG 預覽 2000×780 / vCard / QR SVG）到 ~/Documents/02_街聲/6 名片/SV/{中文姓名}_{英文名去 alias}/。整套流程封裝為 3 個腳本（card_helper.sh / make_card_artifacts.py / place_qr.jsx），全程零 Bash prompt，唯一停下來的點是 Step 6 的 GATE「請確認資訊無誤」。詳細 SOP 與已知問題見 /Users/owner/Claude_Owner/SV/1_名片/SV_名片自動化製作/SOP.md。
---

# SV 名片自動化製作

> 本 skill 是 SOP 的「執行手冊」精簡版。完整原理、已知問題、設計理由見 [`SOP.md`](/Users/owner/Claude_Owner/SV/1_名片/SV_名片自動化製作/SOP.md)
...
### Step 1：建資料夾 + 開檔 + 等就緒
```bash
/Users/owner/mcp-servers/illustrator-mcp-server/scripts/card_helper.sh init "<中文全名>" "<英文名去alias>"
```
...
### Step 8：搬原檔 + 匯 JPG
```bash
/Users/owner/mcp-servers/illustrator-mcp-server/scripts/card_helper.sh save-original "$DEST_DIR" "$BASENAME"
```
...
### Step 10：搬 OL
```bash
/Users/owner/mcp-servers/illustrator-mcp-server/scripts/card_helper.sh save-ol "$DEST_DIR" "$BASENAME"
```

## 📂 最終產出

`~/Documents/02_街聲/6 名片/SV/{中文姓名}_{英文名去alias}/` 內 5 個檔案：
...
| 用途 | 路徑 |
|---|---|
| 模板 .ai | `/Users/owner/Claude_Owner/SV/1_名片/名片範例/一般名片/1_SV/20260522-王小明.ai` |
| Bash 操作合集 | `/Users/owner/mcp-servers/illustrator-mcp-server/scripts/card_helper.sh` |
| vCard + QR + 預處理 | `/Users/owner/mcp-servers/illustrator-mcp-server/scripts/make_card_artifacts.py` |
| QR 置入 + CMYK 染色 | `/Users/owner/mcp-servers/illustrator-mcp-server/scripts/place_qr.jsx` |
| 詳細 SOP（含已知問題深度說明）| `/Users/owner/Claude_Owner/SV/1_名片/SV_名片自動化製作/SOP.md` |
