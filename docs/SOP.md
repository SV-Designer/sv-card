...
要在 Claude Code 中啟動這個流程，跟 Claude 說「**做名片**」（唯一觸發詞）+ 直接拖入或貼上簽呈 PDF 路徑。

> **版型一律依簽呈欄位判斷，不靠觸發語指定**（v0.20.0 行為變更：已移除「做 SV 名片／做中子名片／做台灣中子名片」等指定版型的觸發語，以及「觸發語版型 vs 簽呈版型交叉檢核」保險機制）。詳見 [`SKILL.md`](../skill/SKILL.md) 「觸發 + 版型路由」段。
...
> 🧪 測試紀錄（**已跑通 2 次、輕度畢業**）：(1) v0.16.x 表單 646 / 某同仁申請 — 中子創新 BVI + **改色**（米褐 C0M7Y23K30 + PNG 白底）；(2) v0.17.x 同表單另一版 — 中子文化 + **無改色**（黑白範本 + PNG 灰底 K70）。兩次涵蓋改色/無改色、公司名（中子創新↔中子文化）切換分支，`_F`/`_B` 框名自動對應 11/11。
>
...
| 台灣中子 | ✅ v0.14.0 畢業（v0.12.0 表單 647/王小明 第 1 次 + v0.14.0 第 2 次）|
...
**比對規則**：
- 兩邊欄位值不一致 → 停下與使用者確認
- Claude 看視覺發現 typo（例：英文名 `Ming Wan` 像是 `Ming Wang` 的拼錯）→ 停下與使用者確認
...
| **Bash 操作合集**（init / save / finalize / upload-vcard）| `~/.claude/skills/claude-sv-card/scripts/card_helper.sh` |
