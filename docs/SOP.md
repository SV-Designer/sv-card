...
**簽呈版型欄「經典復刻款 BVI」** → 走此分支。**雙面卡、米褐色系設計**，與其他款差異大，**此款 SOP 有專屬微調**。目前**手動逐步製作**（尚未納入 card_helper 自動化），需跑通 2 次才畢業。模板：`templates/20260622-名片模版_經典款.ai`（雙面並排：正面 `left<500`、背面 `left>=500`；v0.19.0 加寬 `PH_COMPANY` 框以容下台灣中子 12 字全名）。
