...
### 經典復刻款 BVI 版分支（v0.16.x，**測試中**）

**簽呈版型欄「經典復刻款 BVI」** → 走此分支。**雙面卡、米褐色系設計**，與其他款差異大，**此款 SOP 有專屬微調**。目前**手動逐步製作**（尚未納入 card_helper 自動化），需跑通 2 次才畢業。模板：`templates/20260612-王小明_經典款.ai`（雙面並排：正面 `left<500`、背面 `left>=500`）。
...
**配色 SOP（簽呈有顏色備註時，CMYK 例 `C0 M7 Y23 K30`）**：
- **底色 ×2** → 備註色。⚠️ 雙面底色**兩種結構**：正面是名為「底色」的 **PathItem**、背面是名為「底色」的 **GroupItem**（要進群組染內部 PathItem）。
- **`PH_COMPANY_EN_F`**（正面，在白色文字底色條上）→ 備註色（fill + stroke）；**`PH_COMPANY_EN_B`**（背面，在備註色底上）→ **白**（同色否則隱形）。
- **文字底色**（黑直條）→ 白。
- **其餘文字框**（排除備註＆參考線）→ 文字色（fill）改白。
- ⚠️ **描邊只改原本就有 stroke 的文字、保留原粗細**（`strokeColor.typename !== "NoColor"` 才改，**不動 `strokeWeight`**）；原本無描邊的不要加邊框。
...
> 🧪 測試紀錄：v0.16.x 表單 646 / 李大同（White Chen）第 1 次手動跑通（含 `_F`/`_B` 框名驗證自動對應 11/11）。**需第 2 次才畢業**；畢業前每步逐步確認。
...
| 經典復刻款 BVI | ⏳ **測試中**（v0.16.x 表單 646/White Chen 第 1 次手動跑通；需第 2 次才畢業，配色多為手動 + 逐步確認）|
